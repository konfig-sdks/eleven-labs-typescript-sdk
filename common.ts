...
import { readableStreamToString, ElevenLabsError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new ElevenLabsError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new ElevenLabsError(e, e.response?.data)
                }
                if (innerError instanceof ElevenLabsError) {
                    // Got "ElevenLabsError" from the above try block
                    throw innerError;
                }
